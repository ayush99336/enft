...
import { ENFT_PROGRAM_ADDRESS } from '../programs';
...
export type IncrementInstruction<
  TProgram extends string = typeof ENFT_PROGRAM_ADDRESS,
  TAccountEnft extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountEnft extends string
        ? WritableAccount<TAccountEnft>
        : TAccountEnft,
      ...TRemainingAccounts,
    ]
  >;
...
export type IncrementInput<TAccountEnft extends string = string> = {
  enft: Address<TAccountEnft>;
};

export function getIncrementInstruction<
  TAccountEnft extends string,
  TProgramAddress extends Address = typeof ENFT_PROGRAM_ADDRESS,
>(
  input: IncrementInput<TAccountEnft>,
  config?: { programAddress?: TProgramAddress }
): IncrementInstruction<TProgramAddress, TAccountEnft> {
  // Program address.
  const programAddress = config?.programAddress ?? ENFT_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    enft: { value: input.enft ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.enft)],
    programAddress,
    data: getIncrementInstructionDataEncoder().encode({}),
  } as IncrementInstruction<TProgramAddress, TAccountEnft>;
